...
public class GiftPredicate {
    ...
    static {
        register("profession", (json, name) ->
                new ResourceLocation(GsonHelper.convertToString(json, name)), profession -> (villager, stack, player) -> BuiltInRegistries.VILLAGER_PROFESSION.getKey(villager.getProfession()).equals(profession) ? 1.0f : 0.0f);
        ...
        register("has_item", (json, name) ->
                ...
                (villager, stack, player) -> {
                    for (int i = 0; i < villager.getInventory().getContainerSize(); i++) {
                        if (item.test(villager.getInventory().getItem(i))) {
                            return 1.0f;
                        }
                    }
                    return 0.0f;
        ...
        register("item", (json, name) -> {
            ResourceLocation id = new ResourceLocation(GsonHelper.convertToString(json, name));
        ...
        register("biome", (json, name) ->
                new ResourceLocation(GsonHelper.convertToString(json, name)), biome ->
                (villager, stack, player) ->
                        villager.level().getBiome(villager.blockPosition())
                                .unwrap().left().filter(b -> b.location().equals(biome)).isPresent() ? 1.0f : 0.0f
        );
        register("advancement", (json, name) -> new ResourceLocation(GsonHelper.convertToString(json, name)), id -> (villager, stack, player) -> {
            assert player != null;
            Advancement advancement = Objects.requireNonNull(player.getServer()).getAdvancements().getAdvancement(id);
